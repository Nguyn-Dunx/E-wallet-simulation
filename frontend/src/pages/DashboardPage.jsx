...
function SkeletonCard() {
  return (
    <div className="wallet-card skeleton-card">
      ...
      <div
        className="skeleton skeleton-title"
        style={{ width: "65%", height: 36, marginBottom: 24 }}
      />
...
export default function DashboardPage() {
  ...
  const getTxnDirection = useCallback((txn, walletId) => {
    ...
    if (txn.type === "TRANSFER" && walletId) {
      ...
      if (txn.receiverWalletId && String(txn.receiverWalletId) === wid)
        return "IN";
      if (txn.senderWalletId && String(txn.senderWalletId) === wid)
        return "OUT";
    }
    return null;
  }, []);
  ...
  useEffect(() => {
    if (wallet?.id) fetchSummary(wallet.id);
  }, [wallet?.id, fetchSummary]);
