...
export default function TransactionItem({ txn, walletId, onClick }) {
  ...
  const isIncoming =
    ...
    String(txn.receiverWalletId) === String(walletId);
  ...
  const isOutgoingTransfer =
    ...
    walletId &&
    txn.senderWalletId &&
    String(txn.senderWalletId) === String(walletId);
  const isPositive = isDeposit || isIncoming;
  const label = isTransfer
    ? txn.description || (isIncoming ? "Nhan tien" : "Chuyen tien")
  ...
  return (
    <div className="txn-item" onClick={() => onClick?.(txn)}>
      ...
      <div className="txn-amount-wrap">
        <p
          className={`txn-amount ${isPositive ? "txn-amount--positive" : "txn-amount--negative"}`}
        >
          {isPositive ? "+" : "-"}
          {formatCurrency(txn.amount)}
        </p>
        <span
          className={`badge badge-${txn.status === "SUCCESS" ? "success" : txn.status === "FAILED" ? "danger" : "warning"}`}
        >
